...
<div
	class="mx-auto shadow-2xl h-full w-full bg-base-300 flex flex-col justify-center items-center overflow-hidden z-10 relative"
>
	<div class="svg-container top-0 bottom-0 left-0 right-0 absolute -z-10">
	...
	{#if form?.error || $page.error}
		<div class="alert alert-error shadow-lg w-1/3 mb-5">
			<div>
				<svg
					...
					/></svg
				>
				<span>{$page.error ? $page.error.message : 'Credenciales inválidas.'}</span>
			</div>
		</div>
	{/if}
	<form
	...
	>
		...
		<div class="form-control w-full max-w-xs mb-20">
			<label for="password" class="label">
				<span class="label-text {textError}">Contraseña</span>
			</label>
			<input
				type="password"
				placeholder="••••••••••••••"
	...
	<div class="">
		<button
			disabled={loading}
			class="btn btn-ghost hover:bg-transparent hover:text-accent-focus btn-sm text-lg"
		>
